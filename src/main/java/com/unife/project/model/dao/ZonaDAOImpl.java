...
public class ZonaDAOImpl implements ZonaDAO {
    ...
    public void update(Zona zona) {
        String sql = "UPDATE zona SET portataSensore = ?, statoTerreno = ?, sensoreIlluminazione = ?, sensoreUmidita = ?, sensoreTemperatura = ?, sensorePH = ?, sensoreVento = ?, id_piantagione = ? WHERE coordX = ? and coordY = ?";
    ...
    public void delete(Zona zona) {
        String sql = "DELETE FROM zona WHERE coordX = ? and coordY = ?";
    ...
    @Override
    public Zona findById(int coordX, int coordY) {
